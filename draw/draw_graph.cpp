...
void draw_graph::draw_lbf(map<int, vector<int>>x2){
	...
	for (int i = 0; i < x2.size(); i++){
		if (x2[i].size() != 0){
			vector<int>t = x2[i];
			int deviation = 0;
			double x_coordinate = 2500;
			double y_coordinate = 0;
			double prex_coordinate = 2500;
			double prey_coordinate = 0;
			for (int k = 0; k < file_reclaim.total_blocks; k++){
				int num = pow(3, k+1);
				if (t[k] == 0){
					...
					if (t[k] == 1){
						x_coordinate = x_coordinate - (6000 / num);
					}
					else {
						if (t[k] == 2){
							x_coordinate = x_coordinate;
			...
			for (int k = 0; k < file_reclaim.total_blocks; k++){
				...
				if (t[k+1] == 0){
					...
					if (t[k] == 1){
						prex_coordinate = prex_coordinate - (6000 / num);
					}
					else {
						if (t[k] == 2){
							prex_coordinate = prex_coordinate;
					...
					prey_coordinate = prey_coordinate + 200;
				}
			}
			setcolor(RED);
			setlinestyle(SOLID);
			drawline(prex_coordinate, prey_coordinate, x_coordinate, y_coordinate);
			flushinput();
			//cout<<"I draw a line"<<endl;
			if (prex_coordinate==x_coordinate&&prey_coordinate==y_coordinate){
				cout<<"damn"<<endl;
			}
		}
	}
