...
void create_matrix::get_file()
{
	...
	ofstream outfile_b;
	if (coordinate == 1){
		outfile_b.open("b_x.txt");
	}
	else {
		outfile_b.open("b_y.txt");
	}
	...
	outfile_b.close();
	ofstream outfile_Ap;
	outfile_Ap.open("Ap.txt");
	for (int p = 0; p <matl.Ap.size(); p++){
		outfile_Ap << matl.Ap[p] << " ";
	}
	outfile_Ap.close();
	ofstream outfile_Ai;
	outfile_Ai.open("Ai.txt");
	for (int p = 0; p <matl.Ai.size(); p++){
		outfile_Ai << matl.Ai[p] << " ";
	}
	outfile_Ai.close();
	ofstream outfile_Ax;
	outfile_Ax.open("Ax.txt");
	for (int p = 0; p <matl.Ax.size(); p++){
...
linear_parameter create_matrix::mat(vector<vector<double>>Atest, vector<double>B_test){
	...
	cout<<"total nz="<<nz<<endl;
	cout<<"size of ai:"<<Ai_re.size()<<endl;
